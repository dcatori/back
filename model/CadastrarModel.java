...
public class CadastrarModel extends CadastrarController {
    public static void novoCadastro(String nome, String email, String senha) {
        try {
            ...
            if (rstPesquisarEmail.next()) {
                ...
                CadastrarController.fileFullName = "";
                CadastrarController.fileFullPath = "";

                if (CadastrarController.propriaTela == false) {
                    exibirMenu();
    ...
    public static void copyFile() {
        try {
            ...
            if (fileFullPath.length() > 0 && folderFullPath.length() > 0) {
                ...
                CadastrarController.memorizarArquivo(fileFullPathDestination, fileName);
                CadastrarController.mostrarFoto();

                CadastrarController.notificarUsuario("Imagem foi cadastrada com sucesso.");
            } else {
                CadastrarController.notificarUsuario("Ops! Não foi possível selecionar o arquivo. Por favor tente novamente mais tarde.");
            }
        } catch (Exception e) {
            CadastrarController.notificarUsuario("Ops! Houve algum problema com a seleção do arquivo. Por favor tente novamente mais tarde.");
    ...
    public CadastrarModel(TelaPrincipalView telaPrincipal) {
        super(telaPrincipal);
    }
}